<template>
  ...
  <div class="content" v-show="!isLyricsShow">
    ...
    <img
      ...
      class="img_song"
      @click="isLyricsShow = true"
      :class="{ img_song_active: isPlay, img_song_paused: !isPlay }"
    />
  </div>
  <div class="musicLyrics" ref="musicLyrics" v-show="isLyricsShow">
    <p
  ...
  <div class="footer">
    ...
    <div class="footerContent">
      <input type="range" min="0" :max="duration" v-model="currentTime" step="0.02">
    </div>
    <div class="footerOfFooter">
      <svg class="icon" aria-hidden="true">
        <use xlink:href="#icon-repeatOnce2"></use>
      </svg>
      <svg class="icon" aria-hidden="true" @click="toPlay(-1)">
...
export default {
  ...
  computed: {
    ...mapState(["playList", "lyricsList", "currentTime", "playListIndex", "duration"]),
    lyrics: function () {
      let arr;
    ...
    currentTime: function (time) {
      ...
      if (p) {
        if (p && p.offsetTop > 300) {
          this.$refs.musicLyrics.scrollTop = p.offsetTop - 300;
        }
      }
